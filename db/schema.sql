...
-- Universal contributor intake: category/quantity only make sense for
-- pollution_waste-style cleanup reports. A water-quality measurement, a
-- wildlife sighting, or a "Tell Blue Mind" text report has neither, and the
-- controller now derives a sensible category from the AI-inferred subject
-- family (and defaults quantity to 0) instead of forcing a fake value.
ALTER TABLE activities ALTER COLUMN category DROP NOT NULL;
ALTER TABLE activities ALTER COLUMN quantity DROP NOT NULL;
ALTER TABLE activities ALTER COLUMN quantity SET DEFAULT 0;
...
-- Trust-weighted points (spec §14): corroboration/verification bonuses are
-- keyed to an environmental_event, which may have no legacy activity at all
-- (e.g. an action event, or a non-cleanup contribution). NULLs are distinct
-- for the (activity_id, reason) UNIQUE constraint, so this doesn't weaken
-- the existing per-activity uniqueness guarantee.
ALTER TABLE reward_ledger ALTER COLUMN activity_id DROP NOT NULL;
